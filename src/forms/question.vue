    ...
    <b-container>
      <b-progress :max="max" col="8">
        <b-progress-bar
          ...
          v-if="value != 0"
          :label="
            'Congratulation You Have Finshed' + `${((value / max) * 100).toFixed(2)}%`
          "
        ></b-progress-bar>
        <b-progress-bar
          :value="value"
          style="color: black"
          v-if="value == 0"
          :label="'Start' + `${((value / max) * 100).toFixed(2)}%`"
        ></b-progress-bar>
      </b-progress>
    </b-container>
    <br />
...
export default {
  ...
  mounted: function () {
    ...
    if (WorkBook == 1) {
      ...
      console.log(
        "pages : " +
          (getVisionPages().length +
            getHabitPages().length +
            MindProgrammingPages().length)
      );
      this.max =
        getVisionPages().length + getHabitPages().length + MindProgrammingPages().length;
    } else if (WorkBook == 4) {
      console.log(
        "pages : " +
          (getVisionPages().length +
