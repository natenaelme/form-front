<template>
    ...
    <b-form-select style="margin-top:10px" v-model="selected" :options="options"></b-form-select>
    <div>
        <p style="color:red" v-if="notSelected">
            You Have To Select Workbook
        </p>
    </div>
    <CInput
    ...
    <b-button style="float:right" variant="success" @click="submit()">submit</b-button>
...
<table class="table" >
            ...
            <tbody>
                <tr v-for=" (verification,index) in verification" :key="index">
                    <th>{{index+1}}</th>
                    <th>
                        <p v-if="verification.selected == 1">1</p>
                        <p v-if="verification.selected == 2"> 1&2</p>
                        <p v-if="verification.selected == 3">1,2&3 </p>
                        <p v-if="verification.selected == 4">All 4</p>
                    </th>
                    <th style="font-size:2">{{verification.code}}</th>
`               </tr>
            </tbody>
        ...
        submit(){
            ...
            if(this.selected && this.numberOfCodes){
                ...
                for(var i = 0; i < this.numberOfCodes; i++){
                    ...
                    this.verification.push({selected: this.selected,code : key});
                }
            }
        },
            print () {
